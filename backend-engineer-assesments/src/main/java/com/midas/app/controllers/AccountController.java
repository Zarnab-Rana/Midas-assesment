...
public class AccountController implements AccountsApi {
  ...
  /**
   * PATCH /accounts : Update user account, firstName, lastName, email
   *
   * @param updateAccountDto User account details (required)
   * @pathParam accountId ,id of account (required)
   * @return User account created (status code 200)
   */
  @Override
  public ResponseEntity<AccountDto> updateUserAccount(
      String accountId, UpdateAccountDto updateAccountDto) {
    var account = accountService.updateAccount(updateAccountDto, accountId);
